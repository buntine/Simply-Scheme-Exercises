...
(define (err-message type)
  (cond ((equal? type 'num) "argument must be numeric")
        ...
        ((equal? type 'real) "argument must be a real")
        ((equal? type 'reals) "arguments must both be reals")
        ((equal? type 'posreal) "argument must be a positive real")
        ((equal? type 'divide) "arguments must be divisible")
        ((equal? type 'trig) "argument must be in trigometric range")
        ((equal? type 'bool) "argument must be boolean")
        ((equal? type 'bools) "argument must be boolean")
        ((equal? type 'nonempty) "argument must not be empty or null")
        ((equal? type 'int) "argument must be a integer")
        ((equal? type 'ints) "arguments must both be integers")
        ((equal? type 'posint) "argument must be a positive integer")
        ((equal? type 'hoftypes) "argument must be a correct type")
        ((equal? type 'membertypes) "arguments must be valid in conjunction")
        ((equal? type 'wordorsent) "argument must be a word or sentence")
        ((equal? type 'wordorsents) "arguments must both be words or sentences")
        ((equal? type 'words) "arguments must both be words")
        (else "arguments are of an invalid type")))
...
(define *the-functions*
  (list (list '* * 2 two-numbers? (err-message 'nums))
        ...
        (list 'modulo modulo 2 dividable-integers? (err-message 'divide))
        ...
        (list 'or (lambda (x y) (or x y)) 2
              (lambda (x y) (and (boolean? x) (boolean? y))) (err-message 'bools))
        (list 'quotient quotient 2 dividable-integers? (err-message 'divide))
        (list 'random random 1 (lambda (x) (and (integer? x) (> x 0))) (err-message 'posint))
        (list 'remainder remainder 2 dividable-integers? (err-message 'divide))
